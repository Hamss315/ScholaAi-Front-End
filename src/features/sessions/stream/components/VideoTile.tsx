...
export function VideoTile({ stream, muted = false, label, className = '', isLocal = false }: VideoTileProps) {
    ...
    useEffect(() => {
        ...
        // Retry play() — mediasoup tracks can exist before RTP packets
        // actually start flowing, so the first play() call can silently
        // resolve with zero frames. Retrying on loadedmetadata/canplay
        // fixes the "black tile with correct label" symptom.
        const tryPlay = () => {
            video.play().catch(() => {
                // autoplay blocked — will retry on next event, or user gesture required
            });
        };
        ...
        // Also react to the underlying tracks actually becoming live —
        // covers the case where a track is added to the stream after
        // the stream object itself was already assigned (screen share).
    ...
    return (
        <div className={`relative rounded-2xl overflow-hidden bg-[#1a1d2e] ${className}`}>
            {/* video element always mounted — never unmount/remount on stream changes */}
            <video
                ref={videoRef}
                autoPlay
                playsInline
                muted={muted}
                className={`w-full h-full object-cover ${isLocal ? 'scale-x-[-1]' : ''} ${stream && hasFrame ? '' : 'hidden'}`}
            />
            ...
            {(!stream || !hasFrame) && (
                <div className="w-full h-full flex flex-col items-center justify-center gap-3">
                    ...
                    {label && <span className="text-[#4a5568] text-sm">{stream ? 'Connecting…' : label}</span>}
                </div>
            )}

            {/* Label badge */}
            {label && stream && hasFrame && (
                <div className="absolute bottom-3 left-3">
                    <span className="bg-black/60 backdrop-blur-sm text-white text-xs font-medium px-2.5 py-1 rounded-lg">
                        {label}
                    </span>
